...
# remember in our lecture we used this example for inheritance: class Basic_Member(Member):
...
class Student(models.Model):
    ...
    # Optional Field (could be empty)
    program_category = models.CharField(max_length=35, blank=True)
    ...
    gender = models.CharField(
        max_length=1, choices=GENDER_CHOICES, blank=True)  # New concept!

    # graduation_date:
    # We will NOT use DateTimeField() because it will force us to insert the time also!
    # Who wants to enter the time for graduation!!! :-)
    ...
    # To recap/review:
    # In sql database, we can have relations between tables:
    # 1 to many
    # many to many => this one reflects our connection between the studend and the workshop
    # 1 to 1 (rare)
    # That's why it's called RDBMS (R for Relational)
    ...
    # But the company can give them the documents to read and understand then to sign
    ...
    # 3. it works when we just want to output/dispaly a student object in a template page (for testing)
    #    {{ student }}
    # **********************************************************************************************
    ...
    def __str__(self):
        ...
        # But notice that we used the function str() to cast/convert the numeric data to be string
        # return self.first +" "+ self.last + " | Course: " + self.course + " | Average: " + str(self.average)
        # Or we can use the f string format:
        return (f"{self.first} {self.last} | Course: {self.course} | Average: {self.average}")
...
# and it also inherits from models.Model:
class Workshop(models.Model):
    ...
    # Reason why, if we don't specify the default value, we receive this error:
    ...
    # But Django will display it as"Workshop name" in the administrative page (admin)

    # We need to define the relationship between these two models (Py Classes or DB Tables):
    ...
    # Where can we create or add this many-to-many relationship field?
